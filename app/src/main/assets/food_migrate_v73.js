...
// Make the printed-menu food catalog authoritative for fresh installs.
...
const rev=parseInt(get(REV_KEY)||'0',10)||0;if(rev>=73)return;
let old=null;try{old=JSON.parse(get(PRODUCTS_KEY)||'null')}catch(e){}
if(Array.isArray(old)&&old.length){
  // Keep all drinks and user-created custom products, but replace old built-in foods.
